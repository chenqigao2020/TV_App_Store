...
import com.app.util.download.FileDownloader;
...
public class AppDetailDownloadManager {
    ...
    public boolean isInstalled(String appPackageName) {
        try {
            MyApplication.get().getPackageManager().getApplicationInfo(appPackageName, 0);
            return true;
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    class DownloadApp {
        ...
        private FileDownloader fileCallback;
        ...
        public void start() {
            ...
            fileCallback = new FileDownloader();
            fileCallback.download(url, saveDir, tempFileName, new FileDownloader.DownloadListener() {
                @Override
                public void onSuccess(String filePath) {
                    String tempFilePath = saveDir + "/" + tempFileName;
                    String apkPath = getApkFilePath(url);
                    rename(tempFilePath, apkPath);
        ...
        private void rename(String tempPath, String newPath) {
    ...
    private String getSaveDirPath() {
        File file = MyApplication.get().getExternalCacheDir();
        String savePath = file != null ? file.getAbsolutePath() : MyApplication.get().getCacheDir().getAbsolutePath();
        return savePath;
    }
