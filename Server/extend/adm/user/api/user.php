...
*/
	...
	if($act == 'add'){
		...
		$add_res = Db::table('user')->add(['user'=>$user,'pwd'=>$pwd,'appid'=>$appid,'reg_time'=>$reg_time]);
		//die($res); 
		if($add_res){
	...
	if($act == 'muladd'){
		...
		while($i < $num){
		    ...
		    $add_res = Db::table('user')->add(['user'=>$username,'pwd'=>$password,'appid'=>$appid,'vip'=>$vip,'reg_time'=>$reg_time]);
            if($add_res){
                //保存数据到文件
