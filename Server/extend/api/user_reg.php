...
if ($reg_award_num > 0) {
	if ($reg_award == 'vip') {
		...
		$add_res = Db::table('user')->add(['name' => $name, 'user' => $user, 'pwd' => $pwd, 'vip' => $vip, 'inv' => $inv, 'reg_in' => $reg_in, 'reg_ip' => $reg_ip, 'reg_time' => $reg_time, 'appid' => $appid]);
	} else {
		$add_res = Db::table('user')->add(['name' => $name, 'user' => $user, 'pwd' => $pwd, 'fen' => $reg_award_num, 'inv' => $inv, 'reg_in' => $reg_in, 'reg_ip' => $reg_ip, 'reg_time' => $reg_time, 'appid' => $appid]);
	}
} else {
	$add_res = Db::table('user')->add(['name' => $name, 'user' => $user, 'pwd' => $pwd, 'inv' => $inv, 'reg_in' => $reg_in, 'reg_ip' => $reg_ip, 'reg_time' => $reg_time, 'appid' => $appid]);
}
if ($add_res) {
