...
/*
	...
	*/
	...
	// 上传的根目录
	$uploadDirPath = $_SERVER['DOCUMENT_ROOT'] . "/" . $uploadDirName;
	...
	function getServerBaseUrl() {
		...
		// 获取服务器端口（注意：这是服务器实际监听的端口，而非客户端请求的端口，通常一致）
		$port = $_SERVER['SERVER_PORT'] ?? 80;
		// 协议默认端口不显示，非默认端口才拼接
		$portSuffix = (($protocol === 'http' && $port == 80) || ($protocol === 'https' && $port == 443)) 
			? '' 
			: ':' . $port;
		// 完整主机信息（含端口）
		$fullHost = $protocol . '://' . $host . $portSuffix;
		return $fullHost;
	}

	function getHost() {
		$host = $_SERVER['HTTP_HOST']; // 这通常包括主机名和端口号，如果使用了非标准端口
